...
class MutualInfoSystem(nn.Module):  # mutual information used to maximize channel capacity
    def __init__(self):
        ...
        nn.init.normal(self.fc1.weight, std=0.02)  # init weight with normal distribution and mean is 0, std is 0.02
        nn.init.constant(self.fc1.bias, 0)  # init bias with constant num 0
        nn.init.normal(self.fc2.weight, std=0.02)
        nn.init.constant(self.fc2.bias, std=0.02)
        nn.init.normal(self.fc3.weight, std=0.02)
        nn.init.constant(self.fc3.bias, std=0.02)  # which may not be necessary to initialize weight manually
